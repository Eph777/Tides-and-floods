-- ocean_manager.lua
-- Tide and flood controller for realistic_rising_floods: manages periodic tides, rising sea levels,
...
realistic_rising_floods.tide_settings = {
...
if not base_sealevel or base_sealevel == 0 then
	base_sealevel = realistic_rising_floods.settings.ocean.sea_level or 1
	storage:set_int("sealevel", base_sealevel)
end
realistic_rising_floods.sealevel = base_sealevel
...
function realistic_rising_floods.set_sealevel(height)
	height = math.floor(tonumber(height))
	if height == realistic_rising_floods.sealevel then return end
	realistic_rising_floods.sealevel = height
	storage:set_int("sealevel", height)
	minetest.chat_send_all("[realistic_rising_floods] Sea level set to: " .. height)
end
...
minetest.register_globalstep(function(dtime)
	if not realistic_rising_floods.settings.ocean.enabled then return end

	global_time = global_time + dtime
	-- Export global time for buoyancy
	realistic_rising_floods.ocean_time = global_time * (realistic_rising_floods.settings.ocean.wave_speed or 1.0)

	local mode = realistic_rising_floods.tide_settings.mode

	if mode == "rising" then
		-- Rise speed in nodes per second
		local speed_sec = (realistic_rising_floods.tide_settings.rising_speed or 2.0) / 60.0
		local max_h = realistic_rising_floods.tide_settings.rising_max or 15
		local min_h = realistic_rising_floods.tide_settings.rising_min or 1
		...
		if next_y ~= realistic_rising_floods.sealevel then
			realistic_rising_floods.set_sealevel(next_y)
		end

	elseif mode == "periodic" then
		local low = realistic_rising_floods.tide_settings.tide_low or 1
		local high = realistic_rising_floods.tide_settings.tide_high or 5
		local period_sec = (realistic_rising_floods.tide_settings.tide_period or 10.0) * 60.0
...
minetest.register_chatcommand("tides", {
	...
	description = "Configure realistic_rising_floods ocean tides and flooding",
	privs = {sealevel = true},
	func = function(name, param)
		...
		if not sub or sub == "status" then
			local s = realistic_rising_floods.tide_settings
			local status_str = "\n[realistic_rising_floods Tides Status]:"
				.. "\n- Mode: " .. tostring(s.mode)
				.. "\n- Current sea level: " .. tostring(realistic_rising_floods.sealevel)
		...
		if sub == "mode" then
			...
			if m == "manual" or m == "periodic" or m == "rising" then
				realistic_rising_floods.tide_settings.mode = m
				storage:set_string("mode", m)
				if m == "rising" then
					fractional_sealevel = realistic_rising_floods.sealevel
					storage:set_string("fractional_sealevel", tostring(fractional_sealevel))
				end
		...
		elseif sub == "speed" then
			...
			if val then
				realistic_rising_floods.tide_settings.rising_speed = val
		...
		elseif sub == "range" then
			...
			if low and high then
				...
				realistic_rising_floods.tide_settings.tide_low = low
				realistic_rising_floods.tide_settings.tide_high = high
		...
		elseif sub == "period" then
			...
			if val and val > 0 then
				realistic_rising_floods.tide_settings.tide_period = val
		...
		elseif sub == "max" then
			local val = tonumber(args[2])
			if val then
				realistic_rising_floods.tide_settings.rising_max = val
		...
		elseif sub == "min" then
			local val = tonumber(args[2])
			if val then
				realistic_rising_floods.tide_settings.rising_min = val
...
minetest.register_chatcommand("sealevel", {
	...
	func = function(name, param)
		...
		if not val then
			return false, "Current sea level is: " .. tostring(realistic_rising_floods.sealevel)
		else
			realistic_rising_floods.set_sealevel(val)
...
setmetatable(tidesandfloods, {
	__index = function(t, k)
		if k == "sealevel" then
			return realistic_rising_floods.sealevel
		elseif k == "water_level" then
			return realistic_rising_floods.settings.ocean.sea_level
		end
	end,
	__newindex = function(t, k, v)
		if k == "sealevel" then
			realistic_rising_floods.set_sealevel(v)
		end
	end
})
